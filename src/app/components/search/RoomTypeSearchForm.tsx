    ...
    return (
        <div>
            <form onSubmit={handleSearch} className={styles.form}>
                <div className={styles.inputGroup}><label htmlFor="date">Date</label><input type="date" id="date" name="date" value={criteria.date} onChange={handleChange} className={styles.input} min={new Date().toISOString().split('T')[0]} /></div>
                <div className={styles.inputGroup}><label htmlFor="startTime">Start Time</label><select id="startTime" name="startTime" value={criteria.startTime} onChange={handleChange} className={styles.input}>{timeSlots.slice(0, -1).map(slot => (<option key={slot.value} value={slot.value}>{slot.label}</option>))}</select></div>
                <div className={styles.inputGroup}><label htmlFor="endTime">End Time</label><select id="endTime" name="endTime" value={criteria.endTime} onChange={handleChange} className={styles.input}>{availableEndTimes.map(slot => (<option key={slot.value} value={slot.value}>{slot.label}</option>))}</select></div>
                <div className={styles.inputGroup}><label htmlFor="capacity">People</label><input type="number" id="capacity" name="capacity" min="1" value={criteria.capacity} onChange={handleChange} className={styles.input} /></div>
                <button type="submit" className={styles.searchButton} disabled={isLoading || status !== 'authenticated'}>
            ...
            <div className={styles.resultsContainer}>
                {isLoading && <div className={styles.loadingState}><Loader2 className={styles.loaderIcon} /><p>Finding available spaces...</p></div>}
                {!isLoading && hasSearched && !error && results.length === 0 && <div className={styles.emptyState}><h3>No Spaces Found</h3><p>There are no spaces available that match your criteria.</p></div>}
